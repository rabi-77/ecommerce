...
const getMyOrders = asyncHandler(async (req, res) => {
  ...
  const orders = await Order.find(query)
    .sort({ createdAt: -1 })
    .select('_id orderNumber createdAt totalPrice status isPaid isDelivered items shippingAddress');
...
const cancelOrderItem = asyncHandler(async (req, res) => {
  ...
  if (!['pending', 'processing'].includes(order.status)) {
    res.status(400);
    throw new Error('Cannot cancel items in an order that has been shipped or delivered');
  }
...
const returnOrderItem = asyncHandler(async (req, res) => {
  ...
  const allItemsReturnedOrCancelled = order.items.every(item => item.isReturned || item.isCancelled);
...
const generateInvoice = asyncHandler(async (req, res) => {
  ...
  doc.fontSize(20).text('Your E-commerce Store', { align: 'center' });
  doc.moveDown();
  doc.fontSize(10).text('123 Commerce Street, Retail City, 12345', { align: 'center' });
  doc.text('Phone: (123) 456-7890 | Email: support@yourecommerce.com', { align: 'center' });
  doc.moveDown(2);
  ...
  doc.fontSize(12)
    ...
    .text(`$${order.totalPrice.toFixed(2)}`, amountX, totalsY + 65, { font: 'Helvetica-Bold' });
  
  // Add footer
  doc.fontSize(10)
    .text('Thank you for your business!', 50, totalsY + 100, { align: 'center' });
